...
const DEFAULTS: DevSettings = { voice_platform: "vapi", dev_mode_enabled: false };
...
export const useDevSettings = () => {
  ...
  const load = async () => {
    ...
    setLoading(true);
    const { data } = await supabase.from("dev_settings").select("voice_platform,dev_mode_enabled").eq("user_id", user.id).maybeSingle();
    setSettings({
  ...
  const update = async (patch: Partial<DevSettings>) => {
    ...
    await supabase.from("dev_settings").upsert(
      { user_id: user.id, voice_platform: next.voice_platform, dev_mode_enabled: next.dev_mode_enabled },
      { onConflict: "user_id" },
    );
  };
